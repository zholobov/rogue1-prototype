# Stone Dungeon Visual Detail Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the Stone Dungeon theme feel like a physical place with composite surface geometry, procedural props, higher-contrast textures, and wall-mounted torches.

**Architecture:** ThemeData gets new properties for corridor/ceiling textures and prop density. TextureFactory gets 4 new image patterns (flagstone, cobblestone, ashlar, slabs). LevelBuilder is extended with composite floors/walls/ceilings, torch placement, and prop spawning — all gated behind `prop_density > 0.0` so the neon theme is untouched.

**Tech Stack:** Godot 4.6, GDScript, GL Compatibility renderer, GUT testing framework

**Indentation rules:**
- `level_builder.gd` — TABS
- `theme_data.gd`, `texture_factory.gd`, `stone_theme.gd`, `test_theming.gd`, all new files — 4 SPACES

**Spec:** `docs/superpowers/specs/2026-03-24-stone-dungeon-detail-design.md`
...
Run: `godot --headless -s addons/gut/gut_cmdln.gd -gtest=test/unit/test_theming.gd -v`
Expected: 8 new tests FAIL (properties don't exist yet)

- [ ] **Step 3: Add properties to ThemeData**

In `src/themes/theme_data.gd`, add after the existing `Textures` group (after line 103):
...
git commit -m "feat: add ThemeData properties for corridor/ceiling textures and props"
```

---

### Task 2: TextureFactory New Patterns

**Files:**
- Modify: `src/effects/texture_factory.gd:6-24` (generate_for_theme), `src/effects/texture_factory.gd:90-100` (_generate_image match)
- Test: `test/unit/test_theming.gd`

- [ ] **Step 1: Write failing tests for new texture patterns and cache entries**
...
- [ ] **Step 3: Add draw functions for new patterns**

In `src/effects/texture_factory.gd`, add the 4 new pattern cases to the match in `_generate_image()` and add the draw functions:

Update the match block in `_generate_image` (around line 90):
...
Then add the 4 new static functions at the end of the file:

```gdscript
static func _draw_flagstone(img: Image, w: int, h: int, stone_color: Color, mortar_color: Color) -> void:
    # Large irregular rectangular slabs with mortar lines
    ...
    for y in range(h):
        for x in range(w):
            var row = y / slab_h
            # Offset alternating rows by ~40% for irregular look
            ...
            if sx < mortar or sy < mortar:
                ...
                # Slight per-slab color variation using position hash
...
static func _draw_cobblestone(img: Image, w: int, h: int, stone_color: Color, mortar_color: Color) -> void:
    # Smaller rounded stones packed together
...
static func _draw_ashlar(img: Image, w: int, h: int, stone_color: Color, mortar_color: Color) -> void:
    # Large rectangular cut stone blocks with thin mortar
...
static func _draw_slabs(img: Image, w: int, h: int, slab_color: Color, gap_color: Color) -> void:
    # Wide rectangular panels with thin gaps
...
- [ ] **Step 4: Update generate_for_theme to handle corridor_floor and ceiling**

In `src/effects/texture_factory.gd`, update `generate_for_theme()` to add after the monster_skin block:
...
git commit -m "feat: add flagstone, cobblestone, ashlar, slabs texture patterns + corridor/ceiling cache"
```

---

### Task 3: Stone Theme Updates

**Files:**
- Modify: `themes/stone/stone_theme.gd:54-55` (accent_use_palette), `themes/stone/stone_theme.gd:84-108` (texture params)
- Test: `test/unit/test_theming.gd`

- [ ] **Step 1: Write failing tests for updated stone theme**
...
- [ ] **Step 3: Update stone theme**

Replace the texture section and add prop settings in `themes/stone/stone_theme.gd`:

Change `t.accent_use_palette = true` to `t.accent_use_palette = false`.

Replace the `# Textures` section with:
...
```bash
git add themes/stone/stone_theme.gd test/unit/test_theming.gd
git commit -m "feat: update stone theme with high-contrast textures and prop settings"
```

---

### Task 4: LevelBuilder Texture + UV Setup

**Files:**
- Modify: `src/generation/level_builder.gd:14-38` (_init)

This task updates `_init()` to use the new corridor_floor and ceiling texture cache entries, and adds UV scaling to all materials.

- [ ] **Step 1: Update LevelBuilder._init() with new textures and UV scaling**

In `src/generation/level_builder.gd`, replace the `_init()` method (uses TABS):
...
- [ ] **Step 2: Add _apply_uv_scaling call at start of build()**

In the `build()` method, add as the first line inside the function (before `var root = Node3D.new()`):
...
git commit -m "feat: add corridor/ceiling textures and UV scaling to LevelBuilder"
```

---

### Task 5: LevelBuilder Composite Floors

**Files:**
- Modify: `src/generation/level_builder.gd:92-110` (_add_floor)

Replace the single flat floor box with a 2x2 slab grid with mortar gaps, height variation, and occasional cracked slabs. Collision stays as single box.

- [ ] **Step 1: Replace _add_floor with composite slab grid**

In `src/generation/level_builder.gd`, replace the `_add_floor` method. The `prop_density > 0.0` check gates the composite path; otherwise falls back to original flat floor (uses TABS):
...
```bash
git add src/generation/level_builder.gd
git commit -m "feat: add composite slab grid floors with height variation and cracks"
```

---

### Task 6: LevelBuilder Composite Walls

**Files:**
- Modify: `src/generation/level_builder.gd` (_add_wall_block, _add_edge_strips, _place_strip)

Add stone protrusions, wall trim (non-emissive) for `accent_use_palette = false`, and damage spots.

- [ ] **Step 1: Update _add_wall_block with protrusions and damage**

In `src/generation/level_builder.gd`, replace `_add_wall_block` (uses TABS):
...
- [ ] **Step 2: Update build() to pass grid info to _add_wall_block**

In the `build()` method, change the wall block call from:
```gdscript
...
- [ ] **Step 3: Update _add_edge_strips and _place_strip for wall trim branching**

Replace `_add_edge_strips` and `_place_strip` to branch on `accent_use_palette` (uses TABS):
...
- [ ] **Step 4: Gate floor glow behind accent_use_palette**
...
```bash
git add src/generation/level_builder.gd
git commit -m "feat: add composite walls with protrusions, trim, damage spots, and floor glow gating"
```

---

### Task 7: LevelBuilder Composite Ceilings

**Files:**
- Modify: `src/generation/level_builder.gd` (_add_ceiling, build)

Add cross beams and recessed panels. Beams run along X axis, spaced every `ceiling_beam_spacing` tiles in Z. Rooms get a grid pattern (X and Z beams).

- [ ] **Step 1: Replace _add_ceiling with composite version**

In `src/generation/level_builder.gd`, replace `_add_ceiling` (uses TABS):
...
- [ ] **Step 2: Add ceiling beam pass in build()**

In the `build()` method, change the `_add_ceiling` call to pass `is_room`:
```gdscript
			_add_ceiling(root, world_pos, tile_size, is_room)
```

Then add a ceiling beam pass after the main tile loop but before the directional light. Add inside `build()` before the directional light block (uses TABS):
...
- [ ] **Step 3: Add _add_ceiling_beam helper**

Add to `src/generation/level_builder.gd` (uses TABS):
...
```bash
git add src/generation/level_builder.gd
git commit -m "feat: add ceiling beams and recessed panels"
```

---

### Task 8: LevelBuilder Torches

**Files:**
- Modify: `src/generation/level_builder.gd` (build, _add_light)

Replace the grid-modulo light placement with wall-adjacent torches when `prop_density > 0.0`. Keep original path for neon.

- [ ] **Step 1: Add torch placement pass in build()**

In `build()`, replace the existing light placement block:
...
Then add a torch placement pass after the ceiling beam pass (before directional light) in `build()` (uses TABS):
...
```bash
git add src/generation/level_builder.gd
git commit -m "feat: add wall-mounted torch placement for prop-enabled themes"
```

---

### Task 9: LevelBuilder Props (Pillars, Rubble, Room Props)

**Files:**
- Modify: `src/generation/level_builder.gd` (build)

Add prop placement passes for pillars (room corners), rubble (wall-adjacent floors), and functional room props (barrels, crates, chains).

- [ ] **Step 1: Add prop passes in build()**

Add after the torch pass in `build()`, before the directional light (uses TABS):
...
- [ ] **Step 2: Add prop helper methods**
...
```bash
git add src/generation/level_builder.gd
git commit -m "feat: add pillars, rubble, barrels, crates, and chains for prop-enabled themes"
```
...
git commit -m "test: add integration tests for prop density and accent gating"
```
